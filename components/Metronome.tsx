...
const Metronome: React.FC<MetronomeProps> = ({ bpm, setBpm, isSynced, setIsSynced, setScreenFlash }) => {
      ...
      setTimeout(() => {
        ...
        if (navigator.vibrate && isVibrationEnabled && !mutedBeats[beatIndexToSchedule]) {
           if (beatIndexToSchedule === 0) {
              navigator.vibrate(100); // Сильная доля
           } else {
              navigator.vibrate(50); // Остальные доли
           }
        }
      }, timeUntilBeat);
  ...
  return (
    <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-2xl flex flex-col gap-8">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-400 mb-2 text-center">Отключить долю</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-6">
        <div className="sm:col-span-2">
          <label htmlFor="bpm-slider" className="block text-sm font-medium text-slate-400 mb-2">Темп (BPM)</label>
        ...
        <div>
          <label htmlFor="volume" className="block text-sm font-medium text-slate-400 mb-2">Громкость</label>
          <input type="range" id="volume" min="0" max="1" step="0.01" value={volume} onChange={(e) => setVolume(parseFloat(e.target.value))} />
        </div>
        <div>
          <label htmlFor="time-signature" className="block text-sm font-medium text-slate-400 mb-2">Размер</label>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-around items-center border-t border-slate-800 pt-6">
        <div className="flex items-center gap-3">
          <label htmlFor="screen-flash" className="text-sm font-medium text-slate-300">Моргание экрана</label>
        ...
        <div className="flex items-center gap-3">
          <label htmlFor="sync-bpm" className="text-sm font-medium text-slate-300">Синхронизация</label>
        ...
        <div className="flex items-center gap-3">
          <label htmlFor="vibration" className="text-sm font-medium text-slate-300">Вибрация</label>
